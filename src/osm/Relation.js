...
class Relation extends Element {
  ...
  set subRelations(value) {
    for (const subRelation of value) {
      const index = this.members.findIndex(
        m => m instanceof Relation && m.id === subRelation.id
      );
  ...
  get nodes() {
    return [...this.getByType('Node')];
  }

  get ways() {
    return [...this.getByType('Way')];
  }

  get relations() {
    return [...this.getByType('Relation')];
  }
    ...
    if (relation.members) {
      ...
      const relationsMap = new Map(
        relations
          ...
          .map(r => [r.id, new Relation({ relation: r })])
      );
  ...
  combineWays() {
    const nodes = this.nodes;
    const sections = this.createSections();
    this.members = sections.combineWays().reverse();
    this.members.push(...nodes);
  }
